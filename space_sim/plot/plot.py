...
def make_plot(title, short_name, fn, args, line_shape="spline"):
    R_val = result_R.subs(v_args(args))
    T_val = result_T.subs(p_args(args))

    t_sample = np.linspace(0, float(T_val), 99)
    image_sample = lambdify(
        t, fn.to_matrix(N).subs({**p_args(args), T: T_val, R: R_val})
    )
    ...
    fig.add_trace(
        go.Scatter(
            x=list(t_sample),
            y=[image_sample(t_val)[0][0] for t_val in t_sample],
    ...
    fig.add_trace(
        go.Scatter(
            x=list(t_sample),
            y=[image_sample(t_val)[1][0] for t_val in t_sample],
            line_shape=line_shape,
            name=f"{short_name} - y",
        )
    )
    fig.add_trace(
        go.Scatter(
            x=list(t_sample),
            y=[image_sample(t_val)[2][0] for t_val in t_sample],
            line_shape=line_shape,
            name=f"{short_name} - z",
        )
    )
